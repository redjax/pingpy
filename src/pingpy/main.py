...
def _parse_ping_response(output):
    """Parses the output of the ping command to extract the IP address, time, TTL, and success status."""
    # Pattern for Windows format (including 'time<1ms' case)
    windows_pattern = r"Reply from ([\d\.]+): bytes=(\d+) time=(<1ms|\d+ms) TTL=(\d+)"
    # Pattern for Linux/macOS format
    unix_pattern = r"(\d+) bytes from ([\d\.]+): icmp_seq=\d+ ttl=(\d+) time=(\d+\.\d+) ms"
    ...
    if match:
        ...
        time = match.group(3)  # Can be "ms" or "<1ms"
        ttl = match.group(4)
        success = True
        return ip_address, success, bytes_received, time, ttl
    ...
    if match:
        ...
        ip_address = match.group(2)
        ttl = match.group(3)
        time = match.group(4) + "ms"  # Add "ms" for consistency with Windows format
        success = True
        return ip_address, success, bytes_received, time, ttl

    # No match indicates a failure
